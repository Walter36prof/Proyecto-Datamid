...
// $resultado = $sentencia->execute([$idArea]);
 ...
 $stm1=$conex->prepare("SELECT * FROM tsubsidio WHERE idSubsidio=".$IdSubsidio);
 $stm1->execute();
...
$stmt = $conex->prepare("INSERT INTO tsubsidio (sueldoinf,sueldosup,limiteinf,limitesup,subsidio,Anio,tipo) VALUES (".$output[0].", ". $output[1].", ". $output[2].", ". $output[3].",". $output[4].",". $output[5].",'". $output[6]."');");

...
if(isset($_POST['idIsr'])){
    ...
    {  //aqui inserta el nuevo registro en la tabla correspondiente evitando sql injection
        $stmt = $conex->prepare("UPDATE isr SET sueldoinf = ?, sueldosup = ?, limiteinf = ?, limitesup = ?, coutafija = ?, porcentaje = ?, anio = ?, tipo = ? WHERE id_isr = ?;");//manda 0 como inactivo el usuario

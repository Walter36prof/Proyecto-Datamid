...
if(isset($_POST['idSubsidio'])){
    ...
    {  //aqui inserta el nuevo registro en la tabla correspondiente evitando sql injection
        $stmt = $conex->prepare("UPDATE tsubsidioasimilados SET sueldoinf = ?, sueldosup = ? , limiteinf = ?, limitesup = ?,subsidio = ?,Anio = ?,tipo = ? WHERE idSubsidio = ?;");//manda 0 como inactivo el usuario    

<?php
if(!isset($_GET["IdSubsidioAsimi"])) exit();
$IdSubsidioAsimi = $_GET["IdSubsidioAsimi"];
include '../database/db.config.php';
// $sentencia = DBC::get()->prepare("DELETE FROM areas WHERE idArea = ?;");
// $resultado = $sentencia->execute([$idArea]);

 $stm1=$conex->prepare("SELECT * FROM tsubsidioasimilados WHERE idSubsidio=".$IdSubsidioAsimi);
 $stm1->execute();
...
$stmt = $conex->prepare("INSERT INTO tsubsidioasimilados (sueldoinf,sueldosup,limiteinf,limitesup,subsidio,Anio,tipo) VALUES (".$output[0].", ". $output[1].", ". $output[2].", ". $output[3].",". $output[4].",". $output[5].",'". $output[6]."');");
...
if($stmt == TRUE){
	...
	header("Location: ../tsubsidioasimilados.php");
	exit;
}
